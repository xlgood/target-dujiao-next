...
type Product struct {
	...
	UpstreamStockManaged   bool                `gorm:"-" json:"upstream_stock_managed"`                                     // 库存由已映射上游维护，仅用于管理端展示
	UpstreamStockAvailable int64               `gorm:"-" json:"upstream_stock_available"`                                   // 上游可售库存（-1 表示不限，仅用于管理端展示）
	UpstreamStockUnknown   bool                `gorm:"-" json:"upstream_stock_unknown"`                                     // TGX 尚未请求实时库存
