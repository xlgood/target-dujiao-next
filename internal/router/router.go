...
func SetupRouter(cfg *config.Config, c *provider.Container) *gin.Engine {
	...
	apiV1 := r.Group("/api/v1")
	{
		...
		user.Use(UserJWTAuthMiddleware(cfg.UserJWT.SecretKey, c.UserRepo))
		{
			...
			user.GET("/reseller/orders", publicHandler.ListResellerOrders)
			user.GET("/reseller/orders/stats", publicHandler.GetResellerOrderStats)
			user.GET("/reseller/orders/:order_no", publicHandler.GetResellerOrderDetail)
